...
public class UserBrowseController {
	...
	public String categories(@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int size,
			@RequestParam(required = false) String kw, Model model) {
		int pageIndex = Math.max(page - 1, 0);
		Page<Category> categoryPage = categoryService.search(kw,
	...
	// ----- PRODUCTS (VIEW ONLY) -----
	@GetMapping("/products")
	public String products(@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int size,
			@RequestParam(required = false) String q, @RequestParam(required = false) Long categoryId, Model model) {
		int pageIndex = Math.max(page - 1, 0);
		Page<Product> pageData = productService.search(q, categoryId,
				PageRequest.of(pageIndex, size, Sort.by("id").descending()));

		List<Integer> pageNumbers = IntStream.rangeClosed(1, pageData.getTotalPages()).boxed().toList();
